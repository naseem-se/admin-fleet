          ...
          <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
            {journeys.map((journey) => (
              <button
                ...
                className={clsx(
                  'w-full text-left rounded-2xl border bg-white p-4 transition-colors',
                  selectedId === journey.id ? 'border-brand-500 ring-1 ring-brand-500' : 'border-gray-200 hover:border-gray-300 card-hover'
                )}
              >
                ...
                <div className="flex items-center gap-4 text-xs text-gray-400 mt-2">
                  <span className="flex items-center gap-1"><Clock size={12} /> {elapsedSince(journey.start.time)}</span>
                  <span className="flex items-center gap-1"><Gauge size={12} /> Started {journey.start.km} km</span>
                </div>
              </button>
            ))}
          </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden card-hover">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 p-5">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">{new Date(selected.start.time).toLocaleTimeString()}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400 mb-1">Duration</p>
                  <p className="text-sm font-medium text-gray-900">{elapsedSince(selected.start.time)}</p>
                </div>
                {selected.last_location && (
                  <div className="col-span-2 sm:col-span-3">
                    <p className="text-xs text-gray-400 mb-1 flex items-center gap-1"><MapPin size={12} /> Last Update</p>
                    <p className="text-sm font-medium text-gray-900">
                      {new Date(selected.last_location.recorded_at).toLocaleString()}
                    </p>
                  </div>
                )}
              </div>
